...
export function upsertStudents(college, students) {
  ...
  if (backend === 'sqlite') {
    ...
    for (const s of students) { const key = String(s.usernameKey || s.hrUsername || '').toLowerCase(); if (!key) continue; stmt.run({ '@c': college, '@n': s.name || '', '@u': s.hrUsername || '', '@k': key, '@r': s.registerNo || '', '@e': s.email || '', '@d': s.department || '', '@s': s.section || '', '@y': s.year || '', '@p': s.campus || '' }); n++; }
    return { count: n };
  }
  const data = readJson('students.json', { students: [], seq: 0 });
  for (const s of students) {
    const key = String(s.usernameKey || s.hrUsername || '').toLowerCase();
    if (!key) continue;
...
export function getSetting(key) {
  if (backend === 'sqlite') { const r = sql.prepare('SELECT value FROM app_settings WHERE key=?').get(key); return r ? r.value : null; }
  return readJson('settings.json', {})[key] ?? null;
}
export function setSetting(key, value) {
  if (backend === 'sqlite') { sql.prepare('INSERT INTO app_settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value').run(key, String(value)); return; }
  const s = readJson('settings.json', {}); s[key] = String(value); writeJson('settings.json', s);
}

// Distinct department / section / year for a college (for dropdowns).
export function getStudentFacets(college) {
  if (!college) return { departments: [], sections: [], years: [] };
  const rows = listStudents({ college });
  const uniq = (f) => Array.from(new Set(rows.map((r) => r[f]).filter(Boolean))).sort((x, y) => String(x).localeCompare(String(y), undefined, { numeric: true }));
  return { departments: uniq('department'), sections: uniq('section'), years: uniq('year') };
}
...
// Keep only the latest scrape per day, and only the most recent `keepDays` days.
export function pruneScrapes(slug, keepDays = 20) {
  if (!slug) return { deleted: 0 };
  if (backend === 'sqlite') {
    const rows = sql.prepare('SELECT id, substr(created_at,1,10) AS d FROM scrapes WHERE slug=? ORDER BY id DESC').all(slug);
    const seenDay = new Set(); const keepDaySet = new Set(); const del = [];
    for (const r of rows) {
      if (seenDay.has(r.d)) { del.push(r.id); continue; } // same-day duplicate (older)
      seenDay.add(r.d);
      if (keepDaySet.size < keepDays) keepDaySet.add(r.d);
      else del.push(r.id); // day beyond the window
    }
    let n = 0; const stmt = sql.prepare('DELETE FROM scrapes WHERE id=?');
    for (const id of del) n += stmt.run(id).changes;
    return { deleted: n };
  }
  const all = readJson('scrapes.json', { items: [] });
  const mine = all.items.filter((x) => x.slug === slug).sort((a, b) => b.id - a.id);
  const seenDay = new Set(); const keepDaySet = new Set(); const delIds = new Set();
  for (const r of mine) {
    const d = String(r.createdAt).slice(0, 10);
    if (seenDay.has(d)) { delIds.add(r.id); continue; }
    seenDay.add(d);
    if (keepDaySet.size < keepDays) keepDaySet.add(d); else delIds.add(r.id);
  }
  const before = all.items.length;
  all.items = all.items.filter((x) => !(x.slug === slug && delIds.has(x.id)));
  writeJson('scrapes.json', all);
  return { deleted: before - all.items.length };
}
