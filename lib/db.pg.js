...
export async function upsertStudents(college, students) {
  const mapped = (students || []).filter((s) => s.hrUsername).map((s) => ({ college, name: s.name || '', hr_username: s.hrUsername, username_key: String(s.hrUsername).toLowerCase(), register_no: s.registerNo || '', email: s.email || '', department: s.department || '', section: s.section || '', year: s.year || '', campus: s.campus || '' }));
