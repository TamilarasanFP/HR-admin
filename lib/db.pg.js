...
export const pool = new Pool({ connectionString, ssl: { rejectUnauthorized: false }, max: 5 });
...
export async function listStudentsForContest(contestId) {
  const contest = await getContest(contestId); if (!contest) return [];
  const all = await listStudents({ college: contest.college });
  const keys = new Set(await getContestStudentKeys(contestId));
...
export async function upsertStudents(college, students) {
  const recs = (students || []).filter((s) => s.hrUsername).map((s) => ({ college, name: s.name || '', hr_username: s.hrUsername, username_key: String(s.hrUsername).toLowerCase(), register_no: s.registerNo || '', email: s.email || '', department: s.department || '', section: s.section || '', year: s.year || '', campus: s.campus || '' }));
  if (recs.length) {
...
export async function getScrapeSeries(slug) {
  if (!slug) return [];
  const rows = await q('SELECT created_at, payload FROM scrapes WHERE slug=$1 ORDER BY created_at ASC', [slug]);
  const byDay = new Map();
  for (const r of rows) {
    const day = (r.created_at instanceof Date ? r.created_at.toISOString() : String(r.created_at)).slice(0, 10);
    const solved = {};
    for (const u of (r.payload?.users || [])) solved[u.username.toLowerCase()] = u.solved;
